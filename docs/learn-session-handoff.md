...
The version-one client is fixed to `learn`. Production accepts only `https://learn.hara-lang.org/api/auth/callback`; testing accepts only the equivalent testing hostname. Local development may set `HARA_LEARN_HANDOFF_REDIRECT_URI` to a loopback callback.
...
Use a separate random value for testing and production. The same environment-specific value is configured on the matching Learn deployment. It must not be exposed to browser code.
