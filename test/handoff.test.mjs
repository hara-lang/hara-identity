...
const ENV = {
  ...
  HARA_WORLD_HANDOFF_SECRET: HANDOFF_SECRET,
};
const NOW = Date.parse("2026-08-07T00:00:00Z");

function authorizeUrl({ state=randomToken(32), verifier=randomToken(48), redirect="https://world.hara-lang.org/api/auth/callback" }={}) {
  const url = new URL("https://id.hara-lang.org/v1/handoffs/authorize");
  url.searchParams.set("client_id", "world");
...
function basic() {
  return `Basic ${Buffer.from(`world:${HANDOFF_SECRET}`).toString("base64")}`;
}
...
test("publishes a fail-closed handoff discovery document", async () => {
  ...
  assert.equal(body.clients[0].redirectUri, "https://world.hara-lang.org/api/auth/callback");
});

test("requires the central session before issuing a World code", async () => {
...
test("issues a PKCE-bound code and exchanges it once server-to-server", async () => {
  ...
  assert.equal(callback.origin, "https://world.hara-lang.org");
  ...
  const form = new URLSearchParams({
    ...
    redirect_uri: "https://world.hara-lang.org/api/auth/callback",
  });
  ...
  assert.equal(body.handoff.audience, "world");
...
test("rejects cross-environment callbacks and incorrect client credentials", async () => {
  const bad = authorizeUrl({ redirect: "https://world.testing.hara-lang.org/api/auth/callback" });
  ...
  const response = await handle(new Request("https://id.hara-lang.org/v1/handoffs/token", {
    method: "POST",
    headers: { Authorization: `Basic ${Buffer.from("world:wrong").toString("base64")}` },
