...
export async function handle(request, {
  ...
  const url = new URL(request.url);
  try {
    if (url.pathname === DISCOVERY_PATH) return handleDiscovery(request, env);
    if (START_PATHS.has(url.pathname)) return handleStart(request, env);
    if (url.pathname === CALLBACK_PATH) return await handleCallback(request, env, fetchImpl, now);
    if (SESSION_PATHS.has(url.pathname)) return handleSession(request, env, now);
    if (LOGOUT_PATHS.has(url.pathname)) return handleLogout(request, env);
    ...
    const returnTo = safeReturnTo(url.searchParams.get("returnTo") ?? "/", request.url, env);
